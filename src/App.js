...
function App() {
  ...
  return (
    <div className="App">
      ...
      <Operations addition={addition} substraction={substraction}  multiply={multiply} division={division}/>
      {showResult && <div>Result = {result}</div>}
      <div style={{ color: Validate.includes('Error') ? 'red' : 'green' }}>{Validate}</div>
    </div>
  );
}
